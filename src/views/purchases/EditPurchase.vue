<template>
  <v-container
    ...
    grid-list-lg
  >
  <NotPermission v-if="!$can('update', 'Purchases')" />
        ...
        >
          <v-card>
            <v-card-title primary-title>
              <span class="success--text font-weight-bold headline">Editar Compra</span>
            </v-card-title>
            <v-divider />
            <v-card-text
              class="pa-0"
            >
              <v-form
                ...
                v-model="validForm"
                lazy-validation
                @submit.prevent="submitUpdatePurchase"
              >
              <v-container
                  fluid
                  grid-list-lg
          ...
          <v-flex
            ...
            <v-divider class="mb-3" />
                <div class="text-xs-center pb-3">
                  ...
                  <v-btn @click="$router.push({ condition: 'sgcPurchasesList' })">
                    Cancelar
                  </v-btn>
                </div>
              </v-form>
...
export default {
  ...
  created () {
    ...
    this.getPurchase({ purchaseId: this.$route.params.id })
      .then(response => {
        const purchaseInfo = response.data.data
        this.setForm(purchaseInfo)
      })
  },
  ...
  methods: {
    ...
    setForm(purchase){
      ...
      this.form.condition = purchase.condition
      this.form.modality = purchase.modality
      this.form.issue_date = purchase.issue_date
      this.form.expiration_date = purchase.expiration_date
      this.form.purchase_date = purchase.purchase_date
      this.form.advance = purchase.advance
      this.form.total = purchase.total
      this.form.state = purchase.state
      this.form.observation = purchase.observation
      this.form.provider_id = purchase.provider ? purchase.provider.id : ''
    },
    formatDate (date) {
      if (!date) return null
    ...
    submitUpdatePurchase () {
      ...
      this.updatePurchase({
        ...
        .then(response => {
          ...
          this.$router.push({ condition: 'sgcPurchasesList' })
        })
        .catch((error) => {
          this.processingForm = false
